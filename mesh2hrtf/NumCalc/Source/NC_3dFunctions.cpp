...
)
{
	...
	int nsbe = -1, nsfl = 1, i, j, ndie, nsel, idi, nsu, j1, j2, jr = 0, nsf0,
		ngaumax = 6, ngaumin = 2;
	double tolf = 1.3,//toleranz für verhältnis Abstand Fläche(subelement)
...
Begin_Subel:
	...
	faclin *= 0.5; // length of the subelements / length of the original el. //relative Kantenlänge der subelements 2^-j
...
Begin_Idi:
	...
	else // the current subelement need not be further subdivided, store the results
	{
		...
		disfac = 0.5/ratdis;//fläche(subelement)/entfernung/2
		ngsbp[nsbe] = ngaumax;
...
)
{
    ...
    // compute the real part of the spherical Hankel function (see K. Giebermann, "Schnelle Sumationsverfahren zur numerischen Lˆsung von Integralgleichungen f¸r Streuprobleme im R^(3)", pp 87-88)
    dnu = (double)(norder - 1);
